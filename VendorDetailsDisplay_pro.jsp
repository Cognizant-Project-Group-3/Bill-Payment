...
<script>
    $(document).ready(function() { $('#vendortype').change(function(event) {  
        var $vendor_type=$("select#vendortype").val();
           $.get('DisplayServlet',{vtype:$vendor_type}, function(responseJson) {   
...
<li><form action="Logout" method="post"><input type="submit" class="button1 btn-default" name="Logout" value="Logout"> </form></li>				
	...
	<div id="VendorDetails" class="container-fluid text-center bg-grey">
		
		<!-- <h2>Bill Payment System</h2> --> 		
		<h4 class="success" >Registration Successful ! Welcome</h4><br>
		<h5>Note your vendor ID for future reference</h5>
...
<tr class="success"><th>Vendor Id</th><td>${id}</td>

<tr ><th>Name</th><td>${name}</td>

<tr ><th>Registration no</th><td>${regno}</td>
...
<tr ><th>State</th><td>${state}</td>

<tr ><th>Contact no</th><td>${phone}</td>

<tr ><th>Website</th><td>${site}</td>

<tr ><th>Employee Count</th><td>${empcount}</td>

<tr ><th>Year of establishment</th><td>${yoe}</td>
